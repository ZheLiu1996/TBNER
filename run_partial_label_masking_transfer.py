...
if __name__ == '__main__':
    ...
    train_pkl = root + r'train_CDWC.pkl'
    dev_pkl = root + r'dev_CDWC.pkl'
    test_pkl=root + r'test_CDWC.pkl'
    write_path=write_root+'predict_partial_label_masking_transfer/'
    if not os.path.exists(write_path):
        os.makedirs(write_path)
        print(write_path,'succeed')
    predict_path=write_path
    record_path=write_root+'prf_ner_all.txt'
    model_save_path=write_root+'model_partial_label_masking_transfer'
    ...
    model_bert_load_path='model_partial_label_masking/model_bert.pth'
    model_sequence_label_load_path='model_partial_label_masking/model_sequence_label.pth'
    ...
    for epoch in range(epoch_num):
        ...
        for batch in tqdm(train_dataloader):
            ...
            lable_loss=loss_cal(out,label_ids)/torch.sum(input_mask)
            optimizer.zero_grad()
            lable_loss.backward()
            optimizer.step()
        ################验证##############
