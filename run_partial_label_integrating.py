...
if __name__ == '__main__':
    ...
    write_path=write_root+'predict_partial_label_integrating/'
    if not os.path.exists(write_path):
        os.makedirs(write_path)
        print(write_path,'succeed')
    predict_path=write_path
    record_path=write_root+'prf_ner_all.txt'
    model_save_path=write_root+'model_partial_label_integrating'
    ...
    for epoch in range(epoch_num):
        ...
        for batch in tqdm(train_dataloader):
            ...
            # print(out)
            out=out.reshape(-1,tagset_size).unsqueeze(0)
            label_ids1=torch.masked_select(label_ids1,input_mask.byte()).unsqueeze(0)
            label_ids2=torch.masked_select(label_ids2,input_mask.byte()).unsqueeze(0)
            sim=torch.masked_select(sim,input_mask.byte()).unsqueeze(0)
            # print(out.size(),label_ids1.size(),label_ids2.size(),sim.size())
            lable_loss=model_crf(out,label_ids1,label_ids2,sim.long())

            total_loss = torch.add(total_loss, -1*lable_loss)
            count += 1
